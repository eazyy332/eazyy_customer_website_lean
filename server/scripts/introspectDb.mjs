...
async function main() {
  ...
  for (const { table_schema, table_name } of tables) {
    ...
    report.tables.push({ schema: table_schema, name: table_name, columns, indexes, foreignKeys: fkeys });
  }
  ...
  for (const t of report.tables) {
    ...
    for (const c of t.columns) {
      md.push(`| ${c.column_name} | ${c.data_type} | ${c.is_nullable} | ${c.column_default ?? ''} |`);
    }
    if (t.foreignKeys?.length) {
      md.push('\n### Foreign Keys');
      for (const fk of t.foreignKeys) {
        md.push(`- ${fk.column_name} → ${fk.foreign_table_schema}.${fk.foreign_table_name}.${fk.foreign_column_name}`);
      }
    }
    if (t.indexes?.length) {
      md.push('\n### Indexes');
